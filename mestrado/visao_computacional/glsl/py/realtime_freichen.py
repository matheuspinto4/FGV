...
with open("freichen_compute.glsl", "r") as f:
...
# Create window
window = glfw.create_window(1280, 720, "Real-Time Frei-Chen Filter", None, None)
if not window:
    glfw.terminate()
...
out = cv2.VideoWriter('output_freichen.mp4', fourcc, 30.0, (1280, 720))
...
threshold = 0.3  # Adjust for edge sensitivity
...
while not glfw.window_should_close(window):
    ...
    if recording:
    # Read back the rendered frame for video recording
        ...
        frame_array = np.flipud(frame_array)  # Flip vertically to match OpenCV
