...
shader_files = {
    'kuwahara': 'kuwahara_compute.glsl',
    'motion': 'motion_detection.glsl',
    'sobel': 'sobel_compute.glsl',
    'emboss': 'emboss_compute.glsl',
    'freichen': 'freichen_compute.glsl',
    'kmeans': 'kmeans_compute.glsl'
}
...
kmeans_k = 4  # Initial number of clusters
kmeans_centers = np.array([
    [1.0, 0.0, 0.0],  # Red
    [0.0, 1.0, 0.0],  # Green
    [0.0, 0.0, 1.0],  # Blue
    [1.0, 1.0, 1.0]   # White
] + [[0.0, 0.0, 0.0]] * 28, dtype=np.float32).ravel()  # Pad to 16 clusters

for name, program in compute_programs.items():
    glUseProgram(program)
    if name == 'kuwahara':
        glUniform1i(glGetUniformLocation(program, "kernelSize"), 5)
    elif name == 'motion':
        glUniform1f(glGetUniformLocation(program, "threshold"), 0.1)
    elif name == 'sobel':
        glUniform1f(glGetUniformLocation(program, "threshold"), 0.2)
    elif name == 'emboss':
        glUniform1f(glGetUniformLocation(program, "intensity"), 1.0)
    elif name == 'freichen':
        glUniform1f(glGetUniformLocation(program, "threshold"), 0.3)
    elif name == 'kmeans':
        glUniform3fv(glGetUniformLocation(program, "clusterCenters"), 32, kmeans_centers)
        glUniform1i(glGetUniformLocation(program, "k"), kmeans_k)

# Initialize filter state
current_filter = 'none'  # Start with raw feed
chain_filters = False  # Toggle chaining (K-Means + Sobel)
last_kmeans_update = 0.0  # Track last k-means update time
...
def key_callback(window, key, scancode, action, mods):
    global current_filter, chain_filters, recording, out, kmeans_k, last_kmeans_update
    if action == glfw.PRESS or action == glfw.REPEAT:
        if key in [glfw.KEY_0, glfw.KEY_1, glfw.KEY_2, glfw.KEY_3, glfw.KEY_4, glfw.KEY_5, glfw.KEY_6]:
            filters = ['none', 'kuwahara', 'motion', 'sobel', 'emboss', 'freichen', 'kmeans']
            current_filter = filters[key - glfw.KEY_0]
            print(f"Filter: {current_filter.capitalize()}")
        elif key == glfw.KEY_C:
        ...
        elif key == glfw.KEY_UP and current_filter == 'kmeans':
            kmeans_k = min(32, kmeans_k + 1)
            last_kmeans_update = 0.0  # Force immediate update
            print(f"K-Means Clusters: {kmeans_k}")
        elif key == glfw.KEY_DOWN and current_filter == 'kmeans':
            kmeans_k = max(2, kmeans_k - 1)
            last_kmeans_update = 0.0  # Force immediate update
            print(f"K-Means Clusters: {kmeans_k}")
...
while not glfw.window_should_close(window):
    ...
    # Update k-means cluster centers periodically
    if current_filter == 'kmeans' and glfw.get_time() - last_kmeans_update > 5.0:
        # Downsample frame for faster k-means
        sample = cv2.resize(frame_rgb, (100, 100))
        sample = sample.reshape((-1, 3)).astype(np.float32)
        # Run k-means
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 1.0)
        _, labels, centers = cv2.kmeans(sample, kmeans_k, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
        # Normalize centers to [0, 1]
        kmeans_centers = (centers / 255.0).ravel()
        # Pad with zeros if k < 16
        kmeans_centers = np.pad(kmeans_centers, (0, 3 * (32 - kmeans_k)), 'constant')
        # Update shader uniforms
        glUseProgram(compute_programs['kmeans'])
        glUniform3fv(glGetUniformLocation(compute_programs['kmeans'], "clusterCenters"), 32, kmeans_centers)
        glUniform1i(glGetUniformLocation(compute_programs['kmeans'], "k"), kmeans_k)
        last_kmeans_update = glfw.get_time()
        print(f"Updated K-Means centers: {centers.tolist()}")
    ...
    if current_filter == 'none':
        ...
        glUseProgram(compute_programs['sobel'])
