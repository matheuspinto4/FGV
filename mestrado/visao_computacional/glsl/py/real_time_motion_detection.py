...
with open("motion_detection.glsl", "r") as f:
...
# Create window
window = glfw.create_window(1280, 720, "Real-Time Motion Detection", None, None)
if not window:
    glfw.terminate()
...
# Set up quad for rendering (y-axis corrected)
vertices = np.array([
    # Position   # TexCoord
...
# Create textures
current_texture = glGenTextures(1)
previous_texture = glGenTextures(1)
...
threshold = 0.2  # Adjust for sensitivity (0.0 to 1.0)
glUseProgram(compute_program)
glUniform1f(glGetUniformLocation(compute_program, "threshold"), threshold)

# Read initial frame for previous_texture
ret, frame = cap.read()
if not ret:
    cap.release()
    glfw.terminate()
...
while not glfw.window_should_close(window):
    ...
    glBindImageTexture(0, current_texture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F)
...
# Cleanup
cap.release()
glDeleteTextures([current_texture, previous_texture, output_texture])
